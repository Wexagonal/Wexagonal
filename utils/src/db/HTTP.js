  ...
  return async(namespace) => {
    ...
    return {
      read: async(key) => {
        ...
        url.searchParams.set("config", JSON.stringify({ key }));
      ...
      write: async(key, value) => {
        ...
        url.searchParams.set("config", JSON.stringify({ key, value }));
        const res = await (await fetch(url)).json();
      ...
      keys: async() => {
        ...
        const res = await (await fetch(url)).json();
        consola.info(`数据库 keys 耗时: ${new Date().getTime() - t1}ms`);
        return res.data;
      },
      values: async() => {
        url.searchParams.set("action", "values");
        const res = await (await fetch(url)).json();
        consola.info(`数据库 values 耗时: ${new Date().getTime() - t1}ms`);
        return res.data;
      },
